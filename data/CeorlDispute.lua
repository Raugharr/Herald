...
Mission.Load {
	...
	OnTrigger = Mission.AddData("Cerol", Mission.GetRandomPerson(true), "BigGuy"),
	Trigger = {Name = "BigGuy", Triggers = {{Type = "Authority", OpCode = Mission.LessThan, Value = 20}, {Type = "IsLeader", OpCode = Mission.Equal, Value = 0}}},
	Id = "CEROL.3",
	MeanTime = 2
}
...
Mission.Load {
	...
	Options = {
		{
			Text = "Permit it",
			Trigger = Mission.CallById("WERGEL.2", Mission.Sender(), Mission.Owner()),
			Utility = Mission.Normalize(0, 1)
		},
		{
			Text = "Forbit it",
			Trigger = Mission.CallById("WERGEL.3", Mission.Sender(), Mission.Owner()),
			Utility = Mission.Normalize(0, 1)
		}
	},
	Id = "CEROL.4",
