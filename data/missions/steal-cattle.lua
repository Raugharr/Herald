	...
	Options = {
		{
			...
			Trigger = function (Frame)
				--local Success = Frame.Owner:OpposedChallange(Frame.From, Stat.Wit)
				local Success = -1

				if Success >= 0 then
					Mission.FireEvent("STLCT.3", Frame.Owner, Frame.From)	
				else
					Mission.FireEvent("STLCT.2", Frame.Owner, Frame.From)
				end
			end,
...
Mission.Load {
	...
	OnTrigger = function(Frame)
		Frame.Owner:GetFamily():TakeAnimal(Frame.From:GetFamily())
	end,
	OnlyTriggered = false,
}
