...
//NOTE: The amount of memory needed for a farmer seems to be a large percentage of the Family's memory. Perhaps
//This memory should be allocated elsewhere and Family stores a pointer to it to save a lot of memory on non farmers.
struct Family {
	...
	union {
		...
		struct {
			uint8_t ProdIdx;//Which product we are making.
			uint8_t GoodIdx;//Current good we are making to create the product.
		} Craftman;
		struct {
