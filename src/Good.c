...
int InputReqQuantityCmp(const void* One, const void* Two) {
	return  ((struct InputReq*)Two)->Quantity - ((struct InputReq*)One)->Quantity;
}
...
struct GoodBase* GoodLoad(lua_State* State, int Index) {
	...
	if(LuaGetString(State, -1, &Name) == 0) {
		lua_pop(State, 1);
		Log(ELOG_WARNING, "Good does not have a Name parameter!");
		return NULL;
	}
	lua_pop(State, 1);
...
/*	Temp = lua_tostring(State, -1);
	...
	lua_pushstring(State, "Produce");
	lua_rawget(State, Index);
	if(lua_type(State, -1) != LUA_TTABLE) {
		lua_pop(State, 1);
		Log(ELOG_WARNING, "%s does not have a Produce parameter.", Name);
		return NULL;
	}
	lua_pushstring(State, "Time");
	lua_rawget(State, -2);
	if(lua_type(State, -1) != LUA_TNUMBER) {
		lua_pop(State, 1);
		Log(ELOG_WARNING, "%s.Produce does not have a Time parameter.", Name);
		return NULL;
	}
	WorkCost = lua_tointeger(State, -1);
	lua_pop(State, 2);
	...
	Good->WkCost = (uint16_t) WorkCost;
	Good->Price = Good->WkCost + (NUTRITION_DAILY * 2 + NUTRITION_CHILDDAILY);
	if(Return > 0) {
		return Good;
	}
	fail:
...
int GoodLoadInput(lua_State* State, struct GoodBase* Good) {
	...
	for(i = 0; i < List.Size; ++i) {
		...
		Good->Price += ((struct GoodBase*) Good->InputGoods[i]->Req)->Price;
		Itr = Itr->Next;
	}
...
int WeaponBaseLoad(lua_State* State, struct WeaponBase* Weapon) {
	...
	if(strcmp(Type, "Spear") == 0) {
		Weapon->Base.ArmsType = ARMS_SPEAR;
		Weapon->Range = MELEE_RANGE;
		Weapon->RangeAttack = 0;
		goto melee;
	} else if(strcmp(Type, "Sword") == 0) {
		Weapon->Base.ArmsType = ARMS_SWORD;
		Weapon->Range = MELEE_RANGE;
		Weapon->RangeAttack = 0;
		goto melee;
	} else if(strcmp(Type, "Javelin") == 0) {
		Weapon->Base.ArmsType = ARMS_JAVELIN;
		Weapon->Range = 3;
	} else if(strcmp(Type, "Bow") == 0) {
		Weapon->Base.ArmsType = ARMS_BOW;
		Weapon->Range = 5;
		Weapon->MeleeAttack = 0;
	} else if(strcmp(Type, "Axe") == 0) {
		Weapon->Base.ArmsType = ARMS_AXE;
		Weapon->Range = MELEE_RANGE;
		Weapon->RangeAttack = 0;
	} else {
	...
	lua_pop(State, 1);
	if(Weapon->Base.ArmsType == ARMS_BOW)
		return 1;
	melee:
...
int ArmorBaseLoad(lua_State* State, struct ArmorBase* Armor) {
	...
	if(strcmp(Type, "Armor") == 0) {
		Armor->Base.ArmsType = ARMS_BODY;
	} else if(strcmp(Type, "Shield") == 0) {
		Armor->Base.ArmsType = ARMS_SHIELD;
	}
	return 1;
}
...
struct InputReq** GoodBuildList(struct GameWorld* World, const struct Array* Goods, int* Size, int Categories) {
	...
	for(int i = 0; i < TblSize; ++i) {
		...
		if((Categories &Good->Base->Category) != Good->Base->Category && Good->Quantity > 0)
			continue;
		Dep = RBSearch(World->GoodDeps, Good->Base);
...
int GoodCanMake(const struct GoodBase* Good, const struct Array* Arr) {
	int Max = INT_MAX;
	int Quantity = 0;
	struct Good** Tbl = (struct Good**) Arr->Table;
	struct Good* Temp = NULL;
	
	for(int i = 0; i < Good->IGSize; ++i) {
		if((Temp = LinearSearch(Good->InputGoods[i], Tbl, Arr->Size, (int(*)(const void*, const void*))InputReqGoodCmp)) == NULL
...
struct Good* ArrayRemoveGood(struct Array* GoodList, uint32_t Index, uint32_t  Quantity) {
	...
	if(Good->Quantity > Quantity) {
		Good->Quantity = Good->Quantity - Quantity;
		Good = g_GoodCopy[ctz(Good->Base->Category)](Good);
	} else {
