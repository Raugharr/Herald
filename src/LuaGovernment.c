...
static const luaL_Reg g_LuaFuncsGovernment[] = {
	...
	{"CreateTreaty", LuaGovernmentCreateTreaty},
	{"IsPlayerOwned", LuaGovernmentIsPlayerOwned},
	{"SwearFealty", LuaGovernmentSwearFealty},
	{"BecomeIndependent", LuaGovernmentLesserRemove},
	{"ScoreAlliance", LuaGovernmentScoreAlliance},
	{"ScoreFealtyRequest", LuaGovernmentScoreFealtyRequest},
	{"ScoreFealtyDemand", LuaGovernmentScoreFealtyDemand},
	{NULL, NULL}
};
...
const struct LuaEnum g_LuaTreatyEnum[] = {
	{"Alliance", TREATY_ALLIANCE},
	{NULL, 0}
};

const struct LuaEnumReg g_LuaGovernmentEnums[] = {
	{"Faction", NULL,  g_LuaFactionGoalEnum},
	{"Treaty", NULL, g_LuaTreatyEnum},
	{NULL, NULL}
};
...
int	LuaGovernmentCreateTreaty(lua_State* State) {
	struct Government* Owner = LuaCheckClass(State, 1, LOBJ_GOVERNMENT);
	struct Government* Target = LuaCheckClass(State, 2, LOBJ_GOVERNMENT);
	int TreatyType = lua_tointeger(State, 3);
	int Length = lua_tointeger(State, 4);
	struct Treaties* List = GovernmentFindTreaties(Owner, Target);
	struct Treaty* Treaty = CreateTreaty(TreatyType, Length);

	if(List == NULL) {
		List = CreateTreatyList(Owner, Target);
		ArrayInsert_S(&Owner->Treaties, List);
	}
	ArrayInsert_S(&List->Treaties, Treaty);

	List = GovernmentFindTreaties(Target, Owner);
	if(List == NULL) {
		List = CreateTreatyList(Target, Owner);
		ArrayInsert_S(&Target->Treaties, List);
	}
	ArrayInsert_S(&List->Treaties, Treaty);
	return 0;
}
...
int LuaGovernmentIsPlayerOwned(lua_State* State) {
	...
	lua_pushboolean(State, g_GameWorld.Player == Government->Leader);
	return 1;
}

int LuaGovernmentSwearFealty(lua_State* State) {
	struct Government* Child = LuaCheckClass(State, 1, LOBJ_GOVERNMENT);
	struct Government* Parent = LuaCheckClass(State, 2, LOBJ_GOVERNMENT);
	
	GovernmentLesserJoin(Parent, Child);
	return 0;
}
...
int LuaGovernmentLesserRemove(lua_State* State) {
	...
	GovernmentLesserRemove(Government);
	return 0;
}
...
int LuaGovernmentScoreAlliance(lua_State* State) {
	...
	lua_pushinteger(State, ScoreAlliance(Government, Target));
	return 1;
}
...
int LuaGovernmentScoreFealtyRequest(lua_State* State) {
	...
	lua_pushinteger(State, ScoreFealtyRequest(Government, Target));
	return 1;
}
...
int LuaGovernmentScoreFealtyDemand(lua_State* State) {
	...
	lua_pushinteger(State, ScoreFealtyDemand(Government, Target));
	return 1;
}
