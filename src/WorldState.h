...
//#define WSToByte(_State, _Idx, AtomIdx) ((((_State)->State[(_Idx)] & (~(_State)->DontCare[(_Idx)]))) >> ((AtomIdx) * CHAR_BIT) & 0xFF)
#define WSToByte(_State, _Idx, AtomIdx) (((_State)->State[(_Idx)]) >> ((AtomIdx) * CHAR_BIT) & 0xFF)
#define WSAtomOffset(_Atom) (((_Atom) % sizeof(WorldState_t)) * CHAR_BIT)
...
#define WSAtomDontCare(_State, _Idx, _Atom) (((0xFF << ((_Atom) * CHAR_BIT)) & ((_State)->DontCare[_Idx])) != 0)
