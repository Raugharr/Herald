...
void ContainerPosChild(struct Container* Parent, struct Widget* Child, SDL_Point* Pos) {
	int32_t X = 0;//_Parent->Widget.Style->Margins.Left;
	int32_t Y = 0;//_Parent->Widget.Style->Margins.Top;
	...
	Pos->x = X;// + Parent->Widget.Style->Margins.Left;
	Pos->y = Y;// + Parent->Widget.Style->Margins.Top;
}
...
void ContextItemNewChild(struct Container* Parent, struct Widget* Child) {
	...
	ContainerPosChild(Parent, Child, &Pos);
	Child->SetPosition(Child, &Pos);
	//_Parent->VertFocChange = Parent->ChildCt;
}
...
void ContainerShrink(struct Container* Container) {
	...
	for(int i = 0; i < Container->ChildCt; ++i) {
		...
		/*if(((Child->Rect.x - Container->Widget.Rect.x) + Child->Rect.w) > (NewW + XOff)) {
			XOff = Child->Rect.x - Container->Widget.Rect.x;
			NewW = XOff + Child->Rect.w;
		}*/
		if(((Child->Rect.y - Container->Widget.Rect.y) + Child->Rect.h + Margins->Bottom) > NewH)
			...
			//_NewH += Child->Rect.h + Margins->Bottom + Margins->Top;
	}
...
/*	for(int i = 0; i < Parent->ChildCt; ++i) {
		...
		if(Rect->x >= Widget->Rect.x && Rect->x + Rect->w <= Widget->Rect.x + Widget->Rect.w) {
			Parent->Children[i]->Rect.x = Widget->Rect.x + Widget->Rect.w + 1;
		}
	}*/
	return true;
}
