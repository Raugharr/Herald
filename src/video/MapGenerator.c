...
void CreateMiniMap(const struct Tile* Tiles, uint32_t TileLength, const struct QuadTree* Tree, uint32_t SetListMax) {
	SDL_Surface* Surface = SDL_CreateRGBSurface(0, TileLength, TileLength, 32, 0xFF000000, 0x00FF0000, 0x0000FF00, 0x000000FF);
	float* Perlin = calloc(TileLength * TileLength, sizeof(float));
	//SDL_Color Color = {0xD2, 0xB4, 0x8C, 0xFF};	
	
	CreatePerlinNoise(TileLength, TileLength, Perlin);
	for(int x = 0; x < TileLength; ++x) {
		for(int y = 0; y < TileLength; ++y) {
			Perlin[y * TileLength + x] = (Perlin[y * TileLength + x] + 1.0f) / 2;
			if(Perlin[y * TileLength + x] < 0.4f) Perlin[y * TileLength + x] = 0.4f;
		}
	}
	if(SDL_SaveBMP(Surface, "Minimap.bmp")) {
		Log(ELOG_ERROR, "%s", SDL_GetError());
	}
	free(Perlin);
	SDL_FreeSurface(Surface);
